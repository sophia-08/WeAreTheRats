...
static void sensorhubAdvertHdlr(void *cookie, uint8_t tag, uint8_t len,
                                uint8_t *value) {
...
static void executableAdvertHdlr(void *cookie, uint8_t tag, uint8_t len,
                                 uint8_t *value) {
...
int sh2_open(sh2_Hal_t *pHal, sh2_EventCallback_t *eventCallback,
  ...
  shtp_listenChan(pSh2->pShtp, GUID_SENSORHUB, "control", sensorhubControlHdlr,
                  &_sh2);
  shtp_listenChan(pSh2->pShtp, GUID_SENSORHUB, "inputNormal",
                  sensorhubInputNormalHdlr, &_sh2);
  shtp_listenChan(pSh2->pShtp, GUID_SENSORHUB, "inputWake",
                  sensorhubInputWakeHdlr, &_sh2);
  shtp_listenChan(pSh2->pShtp, GUID_SENSORHUB, "inputGyroRv",
                  sensorhubInputGyroRvHdlr, &_sh2);

  // Register EXECUTABLE handlers
  shtp_listenAdvert(pSh2->pShtp, GUID_EXECUTABLE, executableAdvertHdlr, &_sh2);
  shtp_listenChan(pSh2->pShtp, GUID_EXECUTABLE, "device", executableDeviceHdlr,
                  &_sh2);
...
/**
 * @brief Put sensor hub in sleep state by sending SLEEP (2) command on "device"
 * channel.
 *
 * @return SH2_OK (0), on success.  Negative value from sh2_err.h on error.
 */
