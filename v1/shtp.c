...
static void addAdvertListener(shtp_t *pShtp, uint16_t guid,
  ...
  if (pShtp->nextAppListener >= SH2_MAX_APPS)
    return;
...
// This is performed every time the underlying Channel, App, Listener data
// structures are updated. As a result, channel number to callback association
// is fast when receiving packets
...
static void shtpAdvertHdlr(void *cookie, uint8_t tag, uint8_t len,
                           uint8_t *val) {
...
static void addChannel(shtp_t *pShtp, uint8_t chanNo, uint32_t guid,
                       const char *chanName, bool wake) {
...
static void callAdvertHandler(shtp_t *pShtp, uint32_t guid, uint8_t tag,
  ...
  for (int n = 0; n < SH2_MAX_APPS; n++) {
    if (pShtp->appListener[n].guid == guid) {
      ...
      if (pShtp->appListener[n].callback != 0) {
        pShtp->appListener[n].callback(pShtp->appListener[n].cookie, tag, len,
                                       val);
        return;
      }
    }
...
static void processAdvertisement(shtp_t *pShtp, uint8_t *payload,
                                 uint16_t payloadLen) {
  uint16_t x;
  uint8_t tag;
  uint8_t len;
  ...
  while (cursor < payloadLen) {
    ...
    case TAG_NULL:
      ...
      break;
    case TAG_GUID:
      // A new GUID is being established so terminate advertisement process with
      // earlier app, if any.
...
static void shtpCmdListener(void *cookie, uint8_t *payload, uint16_t len,
                            uint32_t timestamp) {
  shtp_t *pShtp = (shtp_t *)cookie;
...
void *shtp_open(sh2_Hal_t *pHal) {
  ...
  // When we open the HAL, it resets the device and adverts are sent
  // automatically. So we go to ADVERT_REQUESTED state.  They are on the way.
...
int shtp_listenChan(void *pInstance, uint16_t guid, const char *chan,
                    shtp_Callback_t *callback, void *cookie) {
  shtp_t *pShtp = (shtp_t *)pInstance;
...
int shtp_listenAdvert(void *pInstance, uint16_t guid,
...
uint8_t shtp_chanNo(void *pInstance, const char *appName,
                    const char *chanName) {
  shtp_t *pShtp = (shtp_t *)pInstance;
...
void shtp_service(void *pInstance) {
  ...
  if (pShtp->advertPhase == ADVERT_NEEDED) {
    pShtp->advertPhase =
        ADVERT_REQUESTED; // do this before send, to avoid recursion.
    int status =
        shtp_send(pShtp, SHTP_CHAN_COMMAND, advertise, sizeof(advertise));
    if (status != SH2_OK) {
