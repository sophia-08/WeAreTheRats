...
LSM6DS3::LSM6DS3(uint8_t busType, uint8_t inputArg) : LSM6DS3Core(busType, inputArg) {
    ...
    settings.gyroSampleRate = 416;   //Hz.  Can be: 13, 26, 52, 104, 208, 416, 833, 1660
    ...
    settings.accelSampleRate = 416;  //Hz.  Can be: 13, 26, 52, 104, 208, 416, 833, 1660, 3330, 6660, 13330
...
status_t LSM6DS3::begin() {
    ...
    //Set the ODR bit
    #if 0 //Sophia: This part of code does not make sense
...
float LSM6DS3::calcAccel(int16_t input) {
    float output = (float)input * 0.061 * (settings.accelRange ) / 1000;
    return output;
}
